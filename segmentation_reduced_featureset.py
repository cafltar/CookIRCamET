...
for hy,version in zip(['2023'],['V1']):
    ...
    selector = SelectFromModel(linmod,threshold="0.25*median")
    ...
    parameters = {'linmod__C':[0.001,0.003,0.01,0.03,0.1,0.3,1,3,10]}
    ...
    feats = np.array(feats_list).reshape((-1,n_feat)).astype(np.float32)
    labels = np.array(labels_list).reshape((-1,1)).astype(np.int32).ravel()
    print(feats.shape)
    print(labels.shape)
