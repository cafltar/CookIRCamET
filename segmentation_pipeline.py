...
if __name__ == "__main__":
    ...
    #{'clf__hidden_layer_sizes': (864, 166, 32)}
    #0.884028070675186 0.8846057581018518
    
    #pca
    pca = PCA(svd_solver='full',n_components=0.999)
    clf = MLPClassifier(max_iter=1000,activation='logistic')
    if model == 'mlp':
        #mlp
        ...
        for layer1 in [4,8]:
            for layer2 in [2,4]:
                layer = (layer1*n_feat,int(np.sqrt(layer1*n_feat*layer2*n_components)),layer2*n_components)
                layers.append(layer)
