...
def rhocsdiff(hc , wc , row , LAI , XE , Zeta , rhosoil ):
    ...
    while psis <= pi / 2:
        ...
        while thetas < pi / 2:
            ...
            rhocs_diff = rhocsdiff + 2 * (2 / pi) * RDIR * np.sin(thetas) * np.cos(thetas) * (pi / 12) * (pi / 12)
            thetas = thetas + pi / 12
        psis = psis + pi / 12
    return rhocs_diff
...
# T1     #Distance from radiometer ellipse footprint center to line
                                             #   tangent to radiometer ellipse for given azimuth and zenith angles (m).
If (np.abs(psi - 90 * pi / 180) < 0.01) Then   #90 degree radiometer azimuth
                                                 T1 = y
                                                 If H > T1 Then  #Chord outside of Ellipse
        Chord = 0
        GoTo 20
    Else            #Case 2 with equilateral triangle
                                                     Chord = X * y * np.atan((np.sqrt(y ** 2 - H ** 2)) / H) - H * X / y * np.sqrt(y ** 2 - H ** 2)
                                                     GoTo 20
                                                 End If
                                             Else
                                                 If np.abs(psi) < 0.01 Then     #ZERO degree radiometer azimuth
        T1 = X
        If H > T1 Then  #Chord outside of Ellipse
                                                         Chord = 0
                                                         GoTo 20
                                                     Else            #Case 4 with equilateral triangle
            Chord = X * y * np.atan((np.sqrt(X ** 2 - H ** 2)) / H) - H * y / X * np.sqrt(X ** 2 - H ** 2)
            GoTo 20
        End If
    Else                        #Radiometer azimuth between ZERO and 90 degrees
                                                     # xtr1 
...
If np.abs(X - y) < 0.001 Then #Ellipse is a circle
                                             # hc 
                                             # yy 
                                             # np.acosyy 
                                             hc = H * np.sin(psi)
                                             yy = hc / X
                                             np.acosyy = np.atan(-yy / np.sqrt(-yy * yy + 1)) + 2 * np.atan(1)
                                             Chord = (X ** 2) * np.acosyy - hc * np.sqrt(X ** 2 - hc ** 2)
                                             GoTo 20
                                             Else
                                             GoTo 10
                                             End If
...
Option Explicit
def fveg(XE , thetar , rawpsi , hc , _
wc , row , LAI , Pr , Vr , _
FOV , thetas , psis ) 
#def fveg to compute the fraction of vegetation appearing in a radiometer footprint
                                             #where the crop rows are modeled as continuous ellipses.
...
#Assign value to pi
    ...
    KBR = (np.sqrt(XE ** 2 + (np.tan(thetar)) ** 2)) / _
...
End def
def fcs(OPT As Integer, Pr , row , theta , rawpsi , _
hc , wc , Dv , FOV , thetas , psis ) 
...
#Determine number of rows appearing in radiometer footprint, make it an odd number,
                                        #and add TWO extra rows either side to account for adjacent row shading
10 # NR As Integer
NR = 2 * (Application.Worksheetdef.RoundUp((2 * T1 * np.sin(psi) / row), 0)) + 1# + 4#
...
yy = hc / X
np.acosyy = np.atan(-yy / np.sqrt(-yy * yy + 1)) + 2 * np.atan(1)
Chord = (X ** 2) * np.acos - hc * np.sqrt(X ** 2 - hc ** 2)
GoTo 20
Else
GoTo 10
End If
